...
% Skapa en reducerad uppsättning polynomiella funktioner
poly_train_features = createReducedPolynomialFeatures(train_features, 2);
poly_test_features = createReducedPolynomialFeatures(test_features, 2);
...
% Softmax Regression med reducerad polynomiell representation
W = zeros(size(poly_train_features, 2), num_classes); % Viktmatris
...
softmaxObjective = @(theta) softmaxCostFunction(theta, poly_train_features, train_labels_numeric, num_classes);
...
W = reshape(opt_params(1:end-num_classes), size(poly_train_features, 2), num_classes);
b = opt_params(end-num_classes+1:end);

% Förutsägelser på testdata
scores = poly_test_features * W + b; % Linjär kombination
...
% Reducerad funktion för att skapa polynomiella funktioner
function poly_features = createReducedPolynomialFeatures(features, degree)
    poly_features = features;
    num_features = size(features, 2);
    for i = 1:num_features
        for j = i:num_features
            if i == j || mod(i + j, 2) == 0 % Begränsa antalet interaktioner
